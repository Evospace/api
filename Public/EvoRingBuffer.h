...
class EvoRingBuffer {
  ...
  const TArray<ElementType> &GetData() const { return Elements; }
  ...
  /**
     ...
     */
  FORCEINLINE ElementType &operator[](uint32 Index) { return Elements[Index & IndexMask]; }
  ...
  /**
     ...
     * @param Index The index of the element to return.
     */
  FORCEINLINE const ElementType &operator[](uint32 Index) const { return Elements[Index & IndexMask]; }

  FORCEINLINE void Set(uint32 Index, InElementType value) { Elements[Index & IndexMask] = value; }
  ...
  /**
     ...
     */
  FORCEINLINE uint32 Capacity() const { return Elements.Num(); }
  ...
  FORCEINLINE ElementType GetLast() const { return Elements[Tail]; }
  ...
  /**
     ...
     */
  FORCEINLINE uint32 GetNextIndex(uint32 CurrentIndex) const { return ((CurrentIndex + 1) & IndexMask); }
  ...
  /**
     ...
     */
  FORCEINLINE uint32 GetPreviousIndex(uint32 CurrentIndex) const { return ((CurrentIndex - 1) & IndexMask); }
